...
public class Utils {
  ...
  public static String topKString(Counter c, int k) {
    List sorted = c.toSortedList();
    StringBuilder b = new StringBuilder();
    for (int i=0; i<k && i<sorted.size(); i++) {
      Object key = sorted.get(i);
      b.append(key + "\t" + c.getCount(key) + "\n");
    }
    return b.toString();
  }
 
  public static String bottomKString(Counter c, int k) {
    List sorted = c.toSortedList();
    StringBuilder b = new StringBuilder();
    for (int i=0; i<k && i<sorted.size(); i++) {
      Object key = sorted.get(sorted.size()-1-i);
      b.append(key + "\t" + c.getCount(key) + "\n");
    }
    return b.toString();
  }
