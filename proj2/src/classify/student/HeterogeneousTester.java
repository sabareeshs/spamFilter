...
import classify.spam.*;
import util.*;

public class HeterogeneousTester {

  private static Classifier classifier = new HeterogeneousNaiveBayesClassifier();
  private static HeterogeneousFeatureExtractor fextractor = new HeterogeneousFeatureExtractor();
  ...
  public static void trainAndTestClassifier() {
    if (trainMessages != null && testMessages != null) {
      ...
      for (EmailMessage message : trainMessages) {
        trainingData.add(new HeterogeneousExample(message.getLabel(), fextractor.extractFeatures(message)));
      }
  ...
  private static void testClassifier() {
    ...
    for (EmailMessage message : testMessages) {
      HeterogeneousExample testInstance = new HeterogeneousExample(null, fextractor.extractFeatures(message));
